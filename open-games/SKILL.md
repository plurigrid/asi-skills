...
### Lagrange-KKT Connection

Nash equilibria are characterized as KKT points of the joint optimization:

```
Player i: maximize u_i(x_i, x_{-i})
          subject to x_i ∈ S_i

Lagrangian: L_i(x, λ_i) = u_i(x) + Σ_j λ_{ij} g_{ij}(x_i)

At Nash equilibrium:
  ∇_{x_i} L_i = 0        (stationarity)
  λ_{ij} g_{ij}(x_i) = 0  (complementary slackness)
```

**Dual interpretation**: Strategy constraints become Lagrange multipliers.
The open game `coplay` operator transports utilities through constraint space.

```haskell
-- Nash as Lagrange fixed point
nashViaLagrange :: OpenGame s t a b -> LagrangianSystem
nashViaLagrange g = LagrangianSystem
  { objective = \x -> sum [utility i x | i <- players]
  , constraints = strategyConstraints g
  , multipliers = dualVariables g
  , equilibrium = kktConditions
  }
```

See: `lagrange-kkt` skill for constrained optimization primitives.
