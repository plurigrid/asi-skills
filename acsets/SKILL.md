...
## C-Set Diagram Corpus

The following diagrams document the category-theoretic foundations of C-Sets, organized by the AlgebraicJulia blog series:

### Part 1: Graph Schemas

| Diagram | File | Description |
|---------|------|-------------|
| **Graph Schema** | `diagrams/part1-graph-schema.svg` | Schema Sch(Graph) = {E ⇉ V} with src, tgt morphisms |
| **Symmetric Equations** | `diagrams/part1-symmetric-eqns.svg` | Equations: inv∘src = tgt, inv∘tgt = src, inv² = id |
| **Symmetric Schema** | `diagrams/part1-symmetric-schema.svg` | Full schema with involution morphism E → E |

### Part 2: Half-Edge Graphs

| Diagram | File | Description |
|---------|------|-------------|
| **Half-Edge Schema** | `diagrams/part2-halfedge-schema.svg` | Schema {H → V, H ⇄ H} with vertex and inv |
| **Rotation Schema** | `diagrams/part2-rotation-schema.svg` | Rotation system σ: H → H around vertices |

### Part 3: Reflexive Graphs & Naturality

| Diagram | File | Description |
|---------|------|-------------|
| **Box Product** | `diagrams/part3-box-product.svg` | G □ H categorical product for reflexive graphs |
| **Naturality Square** | `diagrams/part3-naturality.svg` | Naturality of C-set homomorphisms |
| **Reflexive Equations** | `diagrams/part3-reflexive-eqns.svg` | refl∘src = id, refl∘tgt = id |
| **Reflexive Schema** | `diagrams/part3-reflexive-schema.svg` | Schema with refl: V → E morphism |

### Connection to Structured Decompositions

These diagrams form the **foundation layer** for StructuredDecompositions.jl:

```
C-Set Foundation (acsets skill)
        │
        ▼
┌───────────────────────────────────────┐
│  Sch(Graph) = {E ⇉ V}                 │
│  X: Sch(Graph) → Set                  │  ← Functor = C-Set instance
│  X(V) = vertices, X(E) = edges        │
│  X(src), X(tgt): edges → vertices     │
└───────────────────────────────────────┘
        │
        ▼
Structured Decomposition (structured-decompositions skill)
        │
        ▼
┌───────────────────────────────────────┐
│  d: ∫G → C                            │
│  Bags = d(vertices), Adhesions = d(edges)
│  colim(d) = decomposed object         │
│  Sheaf condition on adhesions         │
└───────────────────────────────────────┘
```

**Key insight**: A structured decomposition is a **C-set valued in C-sets** — the shape graph G defines which "slots" (vertices, edges) receive which sub-ACSets (bags, adhesions).

### GF(3) Triadic Bridging

```
structured-decompositions (-1) ⊗ acsets (0) ⊗ gay-mcp (+1) = 0 ✓
sheaf-cohomology (-1) ⊗ acsets (0) ⊗ colimit-reconstruct (+1) = 0 ✓
bumpus-narratives (-1) ⊗ acsets (0) ⊗ triad-interleave (+1) = 0 ✓
```
...
# The decomposition shape is itself a C-set!
shape = @acset Graph begin
    V = 3; E = 2
    src = [1, 2]
    tgt = [2, 3]  # Path graph: 1—2—3
end

# Bags and adhesions are C-sets valued at shape vertices/edges
d = StrDecomp(shape)
bags(d)       # Vector of ACSets at vertices
adhesions(d)  # Vector of ACSets at edges

# Sheaf condition: local solutions glue on adhesions
decide_sheaf_tree_shape(sheaf_functor, d)
```
...
> **HOLE TYPE**: Learnable accessibility relations between preorders

ACSets inherit Galois connection structure from their schema morphisms. A **Galois connection** between preorders P and Q is a pair of monotone maps f: P → Q and g: Q → P such that:
...
### Example: Ceiling Division Galois Connection

```julia
# f = ⌈-/3⌉ : ℝ → ℤ  (left adjoint)
# g = (3×-) : ℤ → ℝ  (right adjoint)
# ⌈x/3⌉ ≤ y  ⟺  x ≤ 3y
```

### ACSets as Galois-Connected Structures

```
┌─────────────────────────────────────────────────────────────┐
│  Schema C ───────────────────────▶ Schema D                 │
│      │                                   │                  │
│      │ F: C-Set ←──────────────────────│ (right adjoint)   │
│      │            Δ (pullback)          │                  │
│      │                                   │                  │
│      ▼ Σ (left Kan extension)           ▼                  │
│  C-Set ────────────────────────────▶ D-Set                  │
│                                                             │
│  Σ ⊣ Δ  forms Galois connection on functor categories     │
└─────────────────────────────────────────────────────────────┘
```

### Accessibility Implications

Previously **inaccessible worlds** (complex structured decompositions) become **accessible** via Galois connections:

| Obstruction | Resolution via Galois Connection |
|------------|----------------------------------|
| High treewidth | Decompose via left adjoint Σ |
| Non-compositional problem | Lift via right adjoint Δ |
| Missing adhesions | Pullback along restriction maps |
| GF(3) imbalance | Factor through ceiling/floor adjoints |

### Cross-Skill Galois Bridges

```
structured-decompositions ⟵Δ───Σ⟶ acsets
         (decompose)            (compose)
              │                      │
              ▼                      ▼
     FPT decidability      C-Set manipulation
```

**Learnable**: This is a **hole type** — master Galois connections to unlock:
- `structured-decompositions` (tree decomposition ⟷ colimit)
- `sheaf-cohomology` (global sections ⟷ local data)
- `kan-extensions` (Σ ⊣ Δ ⊣ Π)
- `dialectica` (proof ⟷ counterexample)
